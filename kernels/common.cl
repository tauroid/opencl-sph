...
inline void multiplyMatrixScalar(float s, float m[9], float out[9]) {
    out[I_00] = s*m[I_00];
    out[I_01] = s*m[I_01];
    out[I_02] = s*m[I_02];
...
inline void jacobi (float i[6], float threshold, uint iterations, private float o[3], float p[9]) {
...
inline void jacobiInvert(float m[6], float out[6]) {
    float eigenvalues[3];
    float eigenvectors[9];

    jacobi(m, 0, 8, eigenvalues, eigenvectors);

    for (uint i = 0; i < 3; ++i) eigenvalues[i] = 1.0/eigenvalues[i];

    multiplyBothSides(eigenvalues, eigenvectors, out);
}
...
inline float determinant(float m[9]) {
    return   m[I_00]*(m[I_11]*m[I_22] - m[I_12]*m[I_21])
           - m[I_01]*(m[I_10]*m[I_22] - m[I_12]*m[I_20])
           + m[I_02]*(m[I_10]*m[I_21] - m[I_11]*m[I_20]);
}
...
kernel void compute_density (PSO_ARGS) {
    ...
    FOR_PARTICLES_IN_RANGE(i, j,
        ...
        if (dist < smoothingradius) ++num_neighbours[i];
