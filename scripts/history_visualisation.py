...
class Canvas(app.Canvas):
    ...
    def __init__(self, history_data):
        ...
        #self.history_count = history_data['position'].shape[0]

        # Create vertices
        n = history_data['position'].shape[1]
        data = np.zeros(n, [('a_position', np.float32, 3),
                            ('a_bg_color', np.float32, 4),
                            ('a_fg_color', np.float32, 4),
                            ('a_size', np.float32, 1)])
        #data['a_position'][:] = history_data['position'][0]
        ...
        self.translate = np.array([0.0,0.0,5.0])
        ...
        self.program['u_size'] = 5 / self.translate[2]
        ...
        self.mouse_press_angles = np.zeros(2)
    ...
    def on_mouse_wheel(self, event):
        ...
        self.translate[2] = max(2.0, self.translate[2])
        self.view = translate(-self.translate)
    ...
    def on_mouse_press(self, event):
        self.mouse_press = np.array(event.pos)
        self.mouse_press_angles[:] = self.angles
    ...
    def on_mouse_move(self, event):
        ...
        self.angles[0] = -(float(event.pos[0]) - self.mouse_press[0]) / self.size[0] + self.mouse_press_angles[0]
        self.angles[1] = -(float(event.pos[1]) - self.mouse_press[1]) / self.size[1] + self.mouse_press_angles[1]
        self.view = tf.euler_matrix(self.angles[1], self.angles[0], 0).dot(translate(-self.translate))
...
def show_history(history_data):
    c = Canvas(history_data)
    app.run()

if __name__ == "__main__":
    show_history({'position': np.array([[[1, 2, 3], [1, 4, 2], [5, 7, 1]]])})
